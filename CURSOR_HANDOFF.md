...
- 一人1レコード。再診断で最新スコアへupsert
- 最大1000人。超過時は低スコアから削除
- 順位、偏差値、上位%、12ビンのヒストグラムを表示
- やり直しで前回差分を表示
...
`npm test` は本番相当ビルド後に `tests/*.test.mjs` を実行する。2026-08-02時点で17テストすべて成功。
...
- 匿名UIDの既存スコアが増殖せず、最新1件へ更新される
- 試算条件・クイズ回答などの個人データをランキングDBへ新規保存していない
