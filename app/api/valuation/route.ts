...
export async function POST(request: Request) {
  try {
    ...
    await db.prepare(`
      ...
      VALUES (?1, ?2, ?3)
      ON CONFLICT(uid) DO UPDATE SET score = excluded.score, updated_at = excluded.updated_at
    ...
    `).run();
    const rows = await db.prepare("SELECT score FROM hmc_scores ORDER BY score DESC LIMIT 1000").all<{ score: number }>();
    const scores = rows.results.map((row) => Number(row.score)).filter(Number.isFinite);
    const response: ValuationResponse = {
      calculation,
      scoreYen,
      quizCorrect,
      reviews: buildQuizReviews(sets, answers),
      ranking: rankingFromScores(scores, scoreYen),
    };
