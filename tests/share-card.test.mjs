...
test("share cards include rarity, market metaphors, and quiz badges", async () => {
  ...
  assert.match(source, /金融リテラシー・これから本番/);
  assert.doesNotMatch(source, /カモ/);
  assert.match(source, /最高評価/);
  assert.doesNotMatch(source, /ストップ高/);
  assert.match(source, /伸びしろ銘柄・仕込み局面/);
  assert.doesNotMatch(source, /上場廃止勧告・監理銘柄入り/);
  assert.match(source, /市場平均より下。ここから伸ばせる帯/);
  assert.doesNotMatch(source, /TOPIXに負けています/);
  assert.match(source, /市場平均を上回っています/);
  assert.match(source, /次に効く一手/);
  assert.match(source, /おおよそ＋/);
